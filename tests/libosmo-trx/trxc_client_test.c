...
static struct osmo_trxc_client *client_alloc(void)
{
	...
	client = osmo_trxc_client_alloc(test_ctx);
	OSMO_ASSERT(client != NULL);
	osmo_trxc_client_set_tx_msg_cb(client, &tx_msg_cb);
	osmo_trxc_client_set_fatal_error_cb(client, &fatal_error_cb);
	osmo_trxc_client_set_priv(client, "test-priv");
