				    ...
				    const uint8_t *buf, size_t buf_len)
{
	...
	case OSMO_TRXD_BURST_LEN_GMSK:
		...
		break;
	case 0: /* NOPE.req: TRXDv0/v1 have no MTS, the burst payload is simply omitted */
 ...
 *  (for logging), store into a thread-local static buffer.
 *  \param[in] bi BURST.ind to get a human-readable representation of
 *  \returns pointer to a thread-local static buffer */
const char *osmo_trxd_burst_ind_name(const struct osmo_trxd_burst_ind *bi)
{
	static __thread char buf[256];
	return osmo_trxd_burst_ind_name_buf(&buf[0], sizeof(buf), bi);
}

/*! Compose a human-readable representation of the given burst indication
 *  (for logging), store into the given buffer.
 *  \param[out] buf output buffer to store the result
 *  \param[in] buf_len size of the output buffer
 *  \param[in] bi BURST.ind to get a human-readable representation of
 *  \returns pointer to the given buffer */
char *osmo_trxd_burst_ind_name_buf(char *buf, size_t buf_len,
				   const struct osmo_trxd_burst_ind *bi)
{
 ...
 *  request (for logging), store into a thread-local static buffer.
 *  \param[in] br BURST.req to get a human-readable representation of
 *  \returns pointer to a thread-local static buffer */
const char *osmo_trxd_burst_req_name(const struct osmo_trxd_burst_req *br)
{
	static __thread char buf[256];
	return osmo_trxd_burst_req_name_buf(&buf[0], sizeof(buf), br);
}

/*! Compose a human-readable representation of the given burst transmit
 *  request (for logging), store into the given buffer.
 *  \param[out] buf output buffer to store the result
 *  \param[in] buf_len size of the output buffer
 *  \param[in] br BURST.req to get a human-readable representation of
 *  \returns pointer to the given buffer */
char *osmo_trxd_burst_req_name_buf(char *buf, size_t buf_len,
				   const struct osmo_trxd_burst_req *br)
{
	struct osmo_strbuf sb = { .buf = buf, .len = buf_len };
