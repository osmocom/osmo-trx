...
int osmo_trxc_msg_parse(struct osmo_trxc_msg *msg, const char *buf, size_t len)
{
	...
	if (strlen(p) >= sizeof(msg->params))
		return -EMSGSIZE;
	OSMO_STRLCPY_ARRAY(msg->params, p);
...
/*! Compose a human-readable representation of the given message
 * (for logging), store into a thread-local static buffer.
 * \param[in] msg parsed TRXC message to get a human-readable representation of
 * \returns pointer to a thread-local static buffer; NULL on error */
const char *osmo_trxc_msg_name(const struct osmo_trxc_msg *msg)
{
	static __thread char buf[OSMO_TRXC_MSG_BUF_SIZE];
	return osmo_trxc_msg_name_buf(&buf[0], sizeof(buf), msg);
}
 ...
 * \param[in] msg parsed TRXC message to get a human-readable representation of
 * \returns pointer to the given buffer; NULL on error */
char *osmo_trxc_msg_name_buf(char *buf, size_t buf_size,
			     const struct osmo_trxc_msg *msg)
{
	return osmo_trxc_msg_build(buf, buf_size, msg) < 0 ? NULL : buf;
}
 ...
 *  \param[out] buf message buffer to store the result
 *  \param[in] buf_size size of the message buffer
 *  \param[in] fn TDMA frame number (validated to be < GSM_TDMA_HYPERFRAME)
 *  \returns length of the message (excl. '\0') on success; negative on error */
