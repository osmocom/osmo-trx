...
DEFUN(cfg_trxd_pdu_batch, cfg_trxd_pdu_batch_cmd,
	"trxd-pdu-batch (disable|enable)",
	"BURST.ind PDU batching (TRXDv2 and higher, default=enable)\n"
...
static int config_write_trx(struct vty *vty)
{
	...
	if (!trx->cfg.trxd_batch)
		vty_out(vty, " trxd-pdu-batch disable%s", VTY_NEWLINE);
...
int trx_vty_init(struct trx_ctx* trx)
{
	...
	install_element(TRX_NODE, &cfg_trxd_pdu_batch_cmd);
